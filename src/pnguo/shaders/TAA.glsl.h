...
layout(set = 0, binding = 0, rgba32f) readonly uniform image2D ColorBuffer;
layout(set = 0, binding = 1, r32f) readonly uniform image2D DepthBuffer;
layout(set = 0, binding = 2, rgba32f) readonly uniform image2D HistoryBuffer;
layout(set = 0, binding = 3, rg32f) readonly uniform image2D VelocityBuffer;
...
vec2 GetClosestVelocity(vec2 uv, vec2 texelSize, out bool isSkyPixel) {
	...
	for (int y = -1; y <= 1; ++y) {
		for (int x = -1; x <= 1; ++x) {
			...
			float depth = imageLoad(DepthBuffer, ivec2(st)).x;  // SampleLevel 对应 imageLoad 
			if (depth < closestDepth) {
				velocity = imageLoad(VelocityBuffer, ivec2(st)).xy;
				closestDepth = depth;
			}
		}
	}
...
vec3 SampleHistoryCatmullRom(vec2 uv, vec2 texelSize) {
	...
	result += imageLoad(HistoryBuffer, ivec2(texPos0.x, texPos0.y)).xyz * w0.x * w0.y;
	result += imageLoad(HistoryBuffer, ivec2(texPos12.x, texPos0.y)).xyz * w12.x * w0.y;
	result += imageLoad(HistoryBuffer, ivec2(texPos3.x, texPos0.y)).xyz * w3.x * w0.y;

	result += imageLoad(HistoryBuffer, ivec2(texPos0.x, texPos12.y)).xyz * w0.x * w12.y;
	result += imageLoad(HistoryBuffer, ivec2(texPos12.x, texPos12.y)).xyz * w12.x * w12.y;
	result += imageLoad(HistoryBuffer, ivec2(texPos3.x, texPos12.y)).xyz * w3.x * w12.y;

	result += imageLoad(HistoryBuffer, ivec2(texPos0.x, texPos3.y)).xyz * w0.x * w3.y;
	result += imageLoad(HistoryBuffer, ivec2(texPos12.x, texPos3.y)).xyz * w12.x * w3.y;
	result += imageLoad(HistoryBuffer, ivec2(texPos3.x, texPos3.y)).xyz * w3.x * w3.y;
...
void mainCS() {
	...
	dims.xy = imageSize(ColorBuffer);  // 0 为 mip 级别 
	...
	ivec2 tilePos = ivec2(gl_LocalInvocationID.xy);  // gl_LocalInvocationID 对应 SV_GroupThreadID 
	tilePos += RADIUS;
	// 填充共享内存 Tile（多线程并行写入） 
	...
	if (localIndex < TILE_DIM * TILE_DIM / 4) {
		...
		vec3 color0 = imageLoad(ColorBuffer, ivec2(uv1)).xyz;
		vec3 color1 = imageLoad(ColorBuffer, ivec2(uv2)).xyz;
		vec3 color2 = imageLoad(ColorBuffer, ivec2(uv3)).xyz;
		vec3 color3 = imageLoad(ColorBuffer, ivec2(uv4)).xyz;
	...
	for (float y = -RADIUS; y <= RADIUS; ++y) {
		for (float x = -RADIUS; x <= RADIUS; ++x) {
			vec3 neigh = Tap(tilePos + ivec2(x, y));
...
void firstCS()
{
	...
	dims.xy = imageSize(ColorBuffer);
	vec2 texelSize = 1.0f / vec2(dims.xy);
	vec2 uv = (gl_GlobalInvocationID.xy + 0.5f) * texelSize;
	ivec2 tilePos = ivec2(gl_LocalInvocationID.xy);
	tilePos += RADIUS;
