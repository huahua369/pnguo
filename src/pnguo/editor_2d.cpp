...
struct ImDrawCmd
{
	...
	struct draw_path_info_t
	{
		...
		const glm::vec2& TexUvWhitePixel = {};
		...
		std::vector<uint32_t> IdxBuffer;          // Index buffer. Each command consume ImDrawCmd::ElemCount of those
		std::vector<Vertex_d> VtxBuffer;        // Vertex buffer.
...
#define IM_FIXNORMAL2F(VX,VY)               { float d2 = VX*VX + VY*VY; if (d2 > 0.000001f) { float inv_len2 = 1.0f / d2; if (inv_len2 > IM_FIXNORMAL2F_MAX_INVLEN2) inv_len2 = IM_FIXNORMAL2F_MAX_INVLEN2; VX *= inv_len2; VY *= inv_len2; } } (void)0
	...
	void strokePolyline(const glm::vec2* points, const int points_count, draw_path_info_t* dp) {
		...
		const glm::vec2 opaque_uv = dp->TexUvWhitePixel;
		const int count = closed ? points_count : points_count - 1; // 线段数
		const bool thick_line = (dp->thickness > dp->_FringeScale);
		auto thickness = dp->thickness;
		auto col = dp->col;
		if (dp->anti_aliased)
		{
			// Anti-aliased stroke
			const float AA_SIZE = dp->_FringeScale;
			...
			const bool use_texture = dp->tex_lines && (integer_thickness < dp->tex_lines_width_max) && (fractional_thickness <= 0.00001f) && (AA_SIZE == 1.0f);
			...
			auto& _VtxWritePtr = dp->_VtxWritePtr;
			auto& _IdxWritePtr = dp->_IdxWritePtr;
			// Temporary buffer
			// The first <points_count> items are normals at each line point, then after that there are either 2 or 4 temp points for each line point
			dp->TempBuffer.resize(points_count * ((use_texture || !thick_line) ? 3 : 5));
			glm::vec2* temp_normals = dp->TempBuffer.data();
			...
			if (use_texture || !thick_line)
			{
				...
				uint32_t idx1 = dp->_VtxCurrentIdx; // Vertex index for start of line segment
				for (int i1 = 0; i1 < count; i1++) // i1 is the first point of the line segment
				{
					const int i2 = (i1 + 1) == points_count ? 0 : i1 + 1; // i2 is the second point of the line segment
					const uint32_t idx2 = ((i1 + 1) == points_count) ? dp->_VtxCurrentIdx : (idx1 + (use_texture ? 2 : 3)); // Vertex index for end of segment
				...
				if (use_texture)
				{
					...
					glm::vec4 tex_uvs = dp->TexUvLines[integer_thickness];
			...
			else
			{
				...
				{
					const int i2 = (i1 + 1) == points_count ? 0 : (i1 + 1); // i2 is the second point of the line segment
					const uint32_t idx2 = (i1 + 1) == points_count ? dp->_VtxCurrentIdx : (idx1 + 4); // Vertex index for end of segment
			...
			dp->_VtxCurrentIdx += (uint32_t)vtx_count;
		}
		else
		{
	...
	void ConvexPolyFilled(const glm::vec2* points, const int points_count, uint32_t col)
	{
	...
	void ConcavePolyFilled(const glm::vec2* points, const int points_count, uint32_t col)
	{
		if (points_count < 3 || (col & IM_COL32_A_MASK) == 0)
			return;
		...
		if (Flags & ImDrawListFlags_AntiAliasedFill)
		{
			...
			while (triangulator._TrianglesLeft > 0)
			{
				...
				_IdxWritePtr[0] = (uint32_t)(vtx_inner_idx + (triangle[0] << 1)); _IdxWritePtr[1] = (uint32_t)(vtx_inner_idx + (triangle[1] << 1)); _IdxWritePtr[2] = (uint32_t)(vtx_inner_idx + (triangle[2] << 1));
				_IdxWritePtr += 3;
			}
		...
		else
		{
			...
			{
				triangulator.GetNextTriangle(triangle);
				_IdxWritePtr[0] = (uint32_t)(_VtxCurrentIdx + triangle[0]); _IdxWritePtr[1] = (uint32_t)(_VtxCurrentIdx + triangle[1]); _IdxWritePtr[2] = (uint32_t)(_VtxCurrentIdx + triangle[2]);
				_IdxWritePtr += 3;
			}
			_VtxCurrentIdx += (uint32_t)vtx_count;
		}
