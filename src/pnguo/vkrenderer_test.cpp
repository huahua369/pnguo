...
#define HASH_SEED 2166136261
	...
	void SetEssentialDeviceExtensions(DeviceProperties* pDp)
	{
		...
		pDp->AddDeviceExtensionName(VK_KHR_SHADER_FLOAT16_INT8_EXTENSION_NAME);
	}
...
#define t_vector std::vector
	...
	public:
		...
		bool CreateDescriptorSetLayout(std::vector<uint32_t>& descriptorCounts, const VkSampler* pSamplers, VkDescriptorSetLayout* descriptorLayout);
		...
		bool CreateDescriptorSetLayout(int size, const VkSampler* pSamplers, VkDescriptorSetLayout* pDescSetLayout);
	...
	private:
		...
		std::map<size_t, dsl_t> dslayouts;
	};
...
#define DOWNSAMPLEPS_MAX_MIP_LEVELS 12
	...
	private:
		...
		void CreatePipeline_pbr(std::vector<VkVertexInputAttributeDescription>& layout, const DefineList& defines, PBRPrimitives* pPrimitive);
	};
	...
	struct tex_dv
	{
		uint32_t index; VkImageView imageView; VkSampler pSampler;
	};
	...
	private:
		...
		VkDescriptorBufferInfo m_perFrameDesc;
		std::map<size_t, DepthPipe_t> _pipem;
	public:
		...
		void CreatePipeline_Depth(std::vector<VkVertexInputAttributeDescription>& layout, const DefineList& defines, DepthPrimitives* pPrimitive);
	};
...
namespace vkr
{
	...
	void GltfDepthPass::load(VkDevice dev, AsyncPool* pAsyncPool)
	{
		...
		for (uint32_t i = 0; i < materials.size(); i++)
		{
			...
			tfmat->m_defines["doubleSided"] = std::to_string(material.doubleSided);
			...
			if (alphaMode == "MASK")
			{
				...
				if (id >= 0)
				{
					...
					tfmat->_textures.push_back({ 0, textureView, sampler });
				}
			}
		}
		...
		// Load Meshes 
		{
			...
			for (uint32_t i = 0; i < meshes.size(); i++)
			{
				...
				for (uint32_t p = 0; p < primitives.size(); p++)
				{
					...
					ExecAsyncIfThereIsAPool(0, [this, i, p, mesh, &primitive, pPrimitive]()
						{
							// Set Material
							//
							...
							//
							{
								...
								CreatePipeline_Depth(inputLayout, defines, pPrimitive);
	...
	void GltfDepthPass::OnDestroy()
	{
		...
		{
			//vkDestroyDescriptorSetLayout(dev, m_materialsData[i].m_descriptorSetLayout, NULL);
	...
	void GltfDepthPass::CreateDescriptors(int inverseMatrixBufferSize, DefineList* pAttributeDefines, DepthPrimitives* pPrimitive, morph_t* morphing)
	{
		...
		auto h = df.Hash();
		auto& oldp = _pipem[h];
		if (!oldp.m_pipelineLayout)
		{
			m_pResourceViewHeaps->CreateDescriptorSetLayout(pPrimitive->m_pMaterial->m_textureCount, &m_sampler, &oldp.m_texturesDescriptorSetLayout);
			...
			if (oldp.m_texturesDescriptorSetLayout != VK_NULL_HANDLE)
				descriptorSetLayout.push_back(oldp.m_texturesDescriptorSetLayout);
			...
			assert(res == VK_SUCCESS);
			SetResourceName(m_pDevice->GetDevice(), VK_OBJECT_TYPE_PIPELINE_LAYOUT, (uint64_t)oldp.m_pipelineLayout, "GltfDepthPass PL");
		}

		if (pPrimitive->m_pMaterial->_textures.size() && oldp.m_texturesDescriptorSetLayout)
		{
			m_pResourceViewHeaps->AllocDescriptor(oldp.m_texturesDescriptorSetLayout, &pPrimitive->m_pMaterial->m_descriptorSet);
			for (auto& it : pPrimitive->m_pMaterial->_textures)
			{
				SetDescriptorSet(m_pDevice->GetDevice(), it.index, it.imageView, it.pSampler, pPrimitive->m_pMaterial->m_descriptorSet);
			}
		}
		m_pResourceViewHeaps->AllocDescriptor(oldp.m_uniformsDescriptorSetLayout, &pPrimitive->m_descriptorSet);
	...
	//
	//--------------------------------------------------------------------------------------
	void GltfDepthPass::CreatePipeline_Depth(std::vector<VkVertexInputAttributeDescription>& layout, const DefineList& defines, DepthPrimitives* pPrimitive)
	{
		/////////////////////////////////////////////
		// Compile and create shaders
		auto h = defines.Hash();
		...
		assert(res == VK_SUCCESS);
		SetResourceName(m_pDevice->GetDevice(), VK_OBJECT_TYPE_PIPELINE, (uint64_t)oldp.m_pipeline, "GltfDepthPass P");
	...
	void GltfDepthPass::Draw(VkCommandBuffer cmd_buf)
	{
		...
		for (uint32_t i = 0; i < pNodes->size(); i++)
		{
			...
			for (int p = 0; p < pMesh->m_pPrimitives.size(); p++)
			{
				...
				if (!pPrimitive->_pipe || pPrimitive->_pipe->m_pipeline == VK_NULL_HANDLE || pPrimitive->m_pMaterial->m_blending)
					continue;
				...
				vkCmdBindDescriptorSets(cmd_buf, VK_PIPELINE_BIND_POINT_GRAPHICS, pPrimitive->_pipe->m_pipelineLayout, 0, descritorSetCount, descriptorSets, uniformOffsetsCount, uniformOffsets);

				// Bind Pipeline
				//
				vkCmdBindPipeline(cmd_buf, VK_PIPELINE_BIND_POINT_GRAPHICS, pPrimitive->_pipe->m_pipeline);
...
namespace vkr
{
					...
					ExecAsyncIfThereIsAPool(0, [this, i, p, mesh, rtDefines, &primitive, pPrimitive, bUseSSAOMask]()
						{
							...
							/*	auto shash = defines.Hash();
								ms.insert(shash);*/
							CreatePipeline_pbr(inputLayout, defines, pPrimitive);
						});
				}
			}
			//size_t a = 0;
	...
	void GltfPbrPass::CreateDescriptorTableForMaterialTextures(PBRMaterial* tfmat, std::map<std::string, ts_t>& texturesBase, SkyDome* pSkyDome, std::vector<VkImageView>& ShadowMapViewPool, bool bUseSSAOMask)
	{
		...
		// for each entry we create a #define with that texture name that hold the id of the texture. That way the PS knows in what slot is each texture.      
		{
			...
			m_pResourceViewHeaps->CreateDescriptorSetLayout(descriptorCounts, NULL, &tfmat->m_texturesDescriptorSetLayout);
			m_pResourceViewHeaps->AllocDescriptor(tfmat->m_texturesDescriptorSetLayout, &tfmat->m_texturesDescriptorSet);
			uint32_t cnt = 0;
	...
	//
	//--------------------------------------------------------------------------------------
	void GltfPbrPass::CreatePipeline_pbr(std::vector<VkVertexInputAttributeDescription>& layout, const DefineList& defines0, PBRPrimitives* pPrimitive)
	{
		// Compile and create shaders
	...
	void GltfPbrPass::BuildBatchLists(drawables_t* opt, bool bWireframe)
	{
		...
		for (uint32_t i = 0; i < pNodes->size(); i++)
		{
			...
			for (uint32_t p = 0; p < pMesh->m_pPrimitives.size(); p++)
			{
				...
				if (!pPrimitive->_pipe || pPrimitive->_pipe->m_pipeline)
					continue;
...
namespace vkr
{
	...
	bool ResourceViewHeaps::CreateDescriptorSetLayout(int size, const VkSampler* pSamplers, VkDescriptorSetLayout* pDescSetLayout)
	{
		...
		auto h = Hash_p((size_t*)layoutBindings.data(), layoutBindings.size() * sizeof(VkDescriptorSetLayoutBinding));
		dsl_t& d = dslayouts[h];
		if (d.layout != 0)
		{
			if (d.count != size || d.num != size) {
				assert(0);
				return false;
			}
			*pDescSetLayout = d.layout;
			return true;
		}
		CreateDescriptorSetLayout(&layoutBindings, &d.layout);
		if (d.layout)
		{
			d.count = size;
			d.num = size;
	...
	bool ResourceViewHeaps::CreateDescriptorSetLayout(std::vector<uint32_t>& descriptorCounts, const VkSampler* pSamplers, VkDescriptorSetLayout* descriptorLayout)
	{
		std::vector<VkDescriptorSetLayoutBinding> layoutBindings(descriptorCounts.size());
		size_t adc = 0;
		for (int i = 0; i < descriptorCounts.size(); i++)
		...
		if (d.layout != 0)
		{
			if (d.count != descriptorCounts.size() || d.num != adc) {
				assert(0);
				return false;
			}
			*descriptorLayout = d.layout;
			return true;
		}
		CreateDescriptorSetLayout(&layoutBindings, &d.layout);
		if (d.layout)
		{
			d.count = layoutBindings.size();
			d.num = adc;
			*descriptorLayout = d.layout;
		}
		return d.layout;
