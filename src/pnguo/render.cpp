...
#define VKVG_PNG_WRITE_IMG_REQUIREMENTS (VK_FORMAT_FEATURE_TRANSFER_SRC_BIT|VK_FORMAT_FEATURE_TRANSFER_DST_BIT|VK_FORMAT_FEATURE_BLIT_DST_BIT)
	...
	{
		auto memoryProperties = (VkPhysicalDeviceMemoryProperties*)dev->memoryProperties;
		...
		, texture_cx* texture, VkSampler* sampler = nullptr, VkSamplerCreateInfo* info = nullptr)
	{
		...
		if (texture->cap_device_mem_size < memReqs.size)
		{
			...
			do {
				memAlloc.memoryTypeIndex = getMemoryType(_dev, memReqs.memoryTypeBits, dh, &memTypeFound);
		...
		if (sampler)
			createSampler(sampler, info);
#endif
		return memAlloc.allocationSize;
	}
	VkFence createFence(VkFenceCreateFlags flags = VK_FENCE_CREATE_SIGNALED_BIT)
	{
		...
		VkFence fence = 0;
		auto hr = vkCreateFence(_dev->vkdev, &fence_create_info, nullptr, &fence);
		return fence;
	}

	//创建采样器
	bool createSampler(VkSampler* sampler, VkSamplerCreateInfo* info)
	{
		...
		return vkCreateSampler(_dev->vkdev, info ? info : &sampler_create_info, nullptr, sampler) == VK_SUCCESS;
	}
	...
	VkSampler createSampler(VkFilter filter = VK_FILTER_LINEAR, VkSamplerAddressMode addressMode = VK_SAMPLER_ADDRESS_MODE_REPEAT)
	{
		...
		auto hr = vkCreateSampler(_dev->vkdev, &sampler_create_info, nullptr, &sampler);
		return sampler;
	}
	...
	VkSemaphore createSemaphore(VkSemaphoreCreateInfo* semaphoreCreateInfo)
	{
		...
		auto hr = vkCreateSemaphore(_dev->vkdev, semaphoreCreateInfo, nullptr, &semaphore);
		return semaphore;
	}
	...
	void resetCommandBuffers()
	{
		...
		for (auto& it : framebuffers)
		{
			it.depth_stencil._dev = _dev->vkdev;
			if (it.semaphore == VK_NULL_HANDLE)
			{
				it.semaphore = createSemaphore(0);
			}
		}
		for (auto& it : framebuffers)
		{
			it.color._dev = _dev->vkdev;
		}
	}
