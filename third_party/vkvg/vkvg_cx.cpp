...
void dy_device_setupPipelines(VkvgDevice dev);
...
public:
	...
	~glist_cx();
	void clear();
	void insert(size_t pos, geom_cmd_t* c);
...
public:
	...
	paths_t* get_path();
	state_save_t* new_ss(state_save_t* src);
	state_save_t* new_state();
	void free_state(state_save_t* p);
	void swap_state(state_save_t* p, state_save_t* p1);

	void set_dash(const float* dashes, uint32_t num_dashes, float offset);
private:
	void poly_fill(paths_t* ctx, vec4* bounds, vgcmd_t& c);
	...
	void cp_cmdt(vgcmd_t* c, state_save_t* t);
};
...
void vkvg_device_destroy(VkvgDevice dev) {
	...
	for (int i = 0; i < 3; i++)
		vkDestroyPipeline(dev->vkDev, dev->dy_pipe[i], NULL);
#ifdef VKVG_WIRED_DEBUG
...
void dy_device_setupPipelines(VkvgDevice dev) {
	...
	VkFormat depthFormat = dev->stencilFormat;
...
#ifndef __APPLE__
	VK_CHECK_RESULT(vkCreateGraphicsPipelines(dev->vkDev, dev->pipelineCache, 1, &pipelineCreateInfo, NULL,
		&dev->dy_pipe[0]));//pipelinePolyFill
#endif
	...
	dynamicState.dynamicStateCount = 5;
	VK_CHECK_RESULT(vkCreateGraphicsPipelines(dev->vkDev, dev->pipelineCache, 1, &pipelineCreateInfo, NULL,
		&dev->dy_pipe[1]));//pipelineClipping));
	...
	// todo pipe
	VK_CHECK_RESULT(vkCreateGraphicsPipelines(dev->vkDev, dev->pipelineCache, 1, &pipelineCreateInfo, NULL
		, &dev->dy_pipe[2]));//pipe_OVER));

	blendAttachmentState.alphaBlendOp = blendAttachmentState.colorBlendOp = VK_BLEND_OP_SUBTRACT;
	VK_CHECK_RESULT(vkCreateGraphicsPipelines(dev->vkDev, dev->pipelineCache, 1, &pipelineCreateInfo, NULL
		, &dev->dy_pipe[3]));//pipe_SUB));

	colorBlendState.logicOpEnable = VK_TRUE;
	blendAttachmentState.blendEnable = VK_FALSE;
	colorBlendState.logicOp = VK_LOGIC_OP_CLEAR;
	VK_CHECK_RESULT(vkCreateGraphicsPipelines(dev->vkDev, dev->pipelineCache, 1, &pipelineCreateInfo, NULL
		, &dev->dy_pipe[4]));//pipe_CLEAR));
...
void _font_cache_font_extents(VkvgContext ctx, vkvg_font_extents_t* extents) {
	...
	extents->max_x_advance = 0; // TODO 宽度
...
// Even-Odd inside test with stencil buffer implementation.
void vgdev_ctx::poly_fill(paths_t* ctx, vec4* bounds, vgcmd_t& c) {
...
void vgdev_ctx::cp_cmdt(vgcmd_t* c, state_save_t* t)
{
...
state_save_t* vgdev_ctx::new_ss(state_save_t* src)
{
	state_save_t* dst = new_state();
	if (!dst)return dst;
	if (src)
	{
		*dst = *src;
...
state_save_t* vgdev_ctx::new_state()
{
	auto t = (state_save_t*)ac.allocate(sizeof(state_save_t));
	*t = {};
	t->curColor = -1;
	//VkRect2D b = {};
	//b.extent = { ctx->pSurf->width, ctx->pSurf->height };
...
void vgdev_ctx::swap_state(state_save_t* p, state_save_t* p1)
{
	std::swap(*p, *p1);
}
...
void dy_bind_draw_pipeline(VkvgContext ctx, state_save_t* t) {
	switch (t->curOperator) {
	case VKVG_OPERATOR_OVER:
		CmdBindPipeline(ctx->cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, ctx->dev->dy_pipe[2]);// pipe_OVER);
		break;
	case VKVG_OPERATOR_CLEAR:
		CmdBindPipeline(ctx->cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, ctx->dev->dy_pipe[4]);//pipe_CLEAR);
		break;
	case VKVG_OPERATOR_DIFFERENCE:
		CmdBindPipeline(ctx->cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, ctx->dev->dy_pipe[3]);//pipe_SUB);
		break;
	default:
		CmdBindPipeline(ctx->cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, ctx->dev->dy_pipe[2]);//pipe_OVER);
		break;
...
void cmd_draw_full_screen_quad(VkvgContext ctx, vgcmd_t* c, vec4* scissor, VkRect2D* clip)
{
#if defined(DEBUG) && defined(VKVG_DBG_UTILS)
	vkh_cmd_label_start(ctx->cmd, "_draw_full_screen_quad", DBG_LAB_COLOR_FSQ);
#endif
...
void dy_start_cmd_for_render_pass(VkvgContext ctx) {
	//if (ctx->pSurf->img->layout != VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL || ctx->dev->threadAware) {
	//	VkhImage imgMs = ctx->pSurf->imgMS;
	//	if (imgMs != NULL)
	//		vkh_image_set_layout(ctx->cmd, imgMs, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
...
void geoms_ctx::draw_geom(VkCommandBuffer cmd, geom_cmd_t* c) {
	if (!c)return;
	if (c->pipeline)
	{
		auto cp = c->pipeline;
		vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, cp->pipeline);
		vkCmdPushConstants(cmd, cp->pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(glm::mat4), &c->mat);
		push_update_descriptor_set(cmd, cp, c->texture ? c->texture : dev->emptyImg1);
	}
	bind(cmd, c->offset * v2offset, c->ioffset);
	if (c->firstIndex == -1)
		vkCmdDraw(cmd, c->count, 1, c->vertexOffset, 0);
	else
		vkCmdDrawIndexed(cmd, c->count, 1, c->firstIndex, c->vertexOffset, 0);
}
void draw_vg(VkvgContext ctx, vgcmd_t* c, VkRect2D& cuclip)
{
	auto t = c->state;
	if (t)
	{
		dy_bind_draw_pipeline(ctx, t);
	...
	{// 填充
		if (t && t->curFillRule == VKVG_FILL_RULE_EVEN_ODD) {
			CmdBindPipeline(ctx->cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, ctx->dev->dy_pipe[0]);// pipelinePolyFill);
			for (size_t i = 0; i < c->vc; i++)
			{
				vkCmdDraw(ctx->cmd, c->v[i].vertexCount, 1, c->v[i].firstVertex, 0);
			}
			dy_bind_draw_pipeline(ctx, t);
			CmdSetStencilCompareMask(ctx->cmd, VK_STENCIL_FRONT_AND_BACK, STENCIL_FILL_BIT);
			cmd_draw_full_screen_quad(ctx, c, &c->bounds, &cuclip);
			CmdSetStencilCompareMask(ctx->cmd, VK_STENCIL_FRONT_AND_BACK, STENCIL_CLIP_BIT);
		}
		else {
			vkCmdDrawIndexed(ctx->cmd, c->index.y, 1, c->index.x, (int32_t)c->vertex.x, 0);
		}
	}
	break;
	case 1:
	...
	case 2:
	{// 裁剪
		int bw = c->bounds.width; int bh = c->bounds.height;
		if (bw != 0 && bh != 0) {
			cuclip = dc_scissor(ctx, bw < 0 || bh < 0 ? nullptr : &c->bounds);
			break;
		}
		if (c->vc > 0 || c->index.y > 0) {
			...
			if (t && t->curFillRule == VKVG_FILL_RULE_EVEN_ODD) {
				...
				CmdBindPipeline(ctx->cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, ctx->dev->dy_pipe[1]);//pipelineClipping);
			}
			else {
				CmdBindPipeline(ctx->cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, ctx->dev->dy_pipe[1]);//pipelineClipping);
				CmdSetStencilReference(ctx->cmd, VK_STENCIL_FRONT_AND_BACK, STENCIL_FILL_BIT);
			...
			CmdSetStencilCompareMask(ctx->cmd, VK_STENCIL_FRONT_AND_BACK, STENCIL_FILL_BIT);
			CmdSetStencilWriteMask(ctx->cmd, VK_STENCIL_FRONT_AND_BACK, STENCIL_ALL_BIT);
			cmd_draw_full_screen_quad(ctx, c, NULL, 0);
			ctx->curClipState = vkvg_clip_state_clip;
#if defined(DEBUG) && defined(VKVG_DBG_UTILS)
			vkh_cmd_label_end(ctx->cmd);
#endif
...
void vgdev_ctx::set_dev(VkvgDevice d)
{
	if (!gt)
		gt = new glist_cx();
	dev = d;
...
// todo draw command list
void* vgdev_ctx::draw(VkvgContext ctx, void** waitSemaphore)
{
	if (!ctx || _vertex.empty())return 0;// 填充0、描边1、裁剪2
...
int dc_rectangle4(paths_t* ctx, const glm::vec4& d, const glm::vec4& radius) {
	if (!ctx)
		return -1;
	if (d.z <= 0 || d.w <= 0)
		return VKVG_STATUS_INVALID_RECT;
	...
	auto width = rc.z;
	auto height = rc.w;
	float r0 = fmin(width / 2.0f, height / 2.0f);
	r.x = glm::clamp(r.x, 0.0f, r0);
	r.y = glm::clamp(r.y, 0.0f, r0);
	r.z = glm::clamp(r.z, 0.0f, r0);
	r.w = glm::clamp(r.w, 0.0f, r0);
	dc_move_to(cr, x + r.x, y);
	dc_line_to(cr, x + width - r.y, y);
	if (r.y > 0)
		dc_arc(cr, x + width - r.y, y + r.y, r.y, 3 * M_PI / 2, 2 * M_PI);
	dc_line_to(cr, x + width, y + height - r.z);
	if (r.z > 0)
		dc_arc(cr, x + width - r.z, y + height - r.z, r.z, 0, M_PI / 2);
	dc_line_to(cr, x + r.w, y + height);
	if (r.w > 0)
		dc_arc(cr, x + r.w, y + height - r.w, r.w, M_PI / 2, M_PI);
	dc_line_to(cr, x, y + r.x);
	if (r.x > 0)
		dc_arc(cr, x + r.x, y + r.x, r.x, M_PI, 3 * M_PI / 2.0);
	dc_close_path(cr);
	return VKVG_STATUS_SUCCESS;
}
...
void dc_get_matrix(vgdev_ctx* ctx, vkvg_matrix_t* matrix) {
	if (!ctx || !ctx->t || !matrix)
		return;
	*matrix = ctx->t->pushConsts.mat;
}
...
void dc_set_color4(vgdev_ctx* ctx, const glm::vec4& c) {
	dc_set_source_rgba(ctx, c.x, c.y, c.z, c.w);
}
...
void rvg_x::add_line(glm::vec4* p, size_t count) {
	if (p && count > 0 && ctx) {
		...
		for (size_t i = 0; i < count; i++)
		{
			dc_move_to(ph, t->x, t->y);
			dc_line_to(ph, t->z, t->w);
			t++;
		}
	}
}
void rvg_x::add_line(const glm::vec2& ps0, const glm::vec2& ps1) {
	if (ctx) {
		auto ph = ctx->get_path();
		dc_move_to(ph, ps0.x, ps0.y);
		dc_line_to(ph, ps1.x, ps1.y);
	}
}
...
void rvg_x::add_rect(const glm::vec4& rc, double r) {
	if (ctx) {
		auto ph = ctx->get_path();
		dc_rectangle(ph, rc.x, rc.y, rc.z, rc.w, r);
	}
}
void rvg_x::add_rect(const glm::vec4& rc, const glm::vec4& r) {
	if (ctx) {
		auto ph = ctx->get_path();
		dc_rectangle4(ph, rc, r);
	}
}
...
void rvg_x::add_triangle(const glm::vec2& pos, const glm::vec2& size, const glm::vec2& dirspos) {
	...
	dc_move_to(ph, pos.x + tpos[0].x, pos.y + tpos[0].y);
	dc_line_to(ph, pos.x + tpos[1].x, pos.y + tpos[1].y);
	dc_line_to(ph, pos.x + tpos[2].x, pos.y + tpos[2].y);
	dc_close_path(ph);
}
// 折线,polygon=closed
void rvg_x::draw_polyline(const glm::vec2& pos, const glm::vec2* points, int points_count, uint32_t col, bool closed, float thickness) {
	auto ph = ctx->get_path();
	if (!ph || !points || points_count < 2)return;
	dc_set_line_width(ctx, thickness);
	dc_move_to(ph, points->x + pos.x, points->y + pos.x);
	for (size_t i = 1; i < points_count; i++)
	{
		dc_line_to(ph, points[i].x + pos.x, points[i].y + pos.x);
	}
	if (closed) { dc_close_path(ph); }
	if (col) {
		dc_set_color(ctx, col);
		ctx->stroke(ph);
	}
}
...
void rvg_x::draw_polylines(const glm::vec2& pos, const glm::vec2* points, int points_count, int* idx, int idx_count, uint32_t col, float thickness) {
	auto ph = ctx->get_path();
	if (!ph || !points || points_count < 2 || idx_count < 2)return;
	...
	for (size_t i = 0; i < idx_count; i++)
	{
		...
		auto ps = points[x] + pos;
		if (nc == 0)
			dc_move_to(ph, ps.x, ps.y);
		else
			dc_line_to(ph, ps.x, ps.y);
		nc++;
	}
	if (nc > 1 && col) {
		ctx->stroke(ph);
...
VkvgPattern new_cubic_gradient(vgdev_ctx* ctx, glm::vec4 rect, glm::vec4 from, glm::vec4 to, glm::vec2 ctrl1, glm::vec2 ctrl2, glm::vec2 p0 = {}, glm::vec2 p1 = { 1,1 }, int steps = 8);
VkvgPattern new_cubic_gradient(vgdev_ctx* ctx, glm::vec4 rect, glm::vec4 from, glm::vec4 to, glm::vec2 ctrl1, glm::vec2 ctrl2, glm::vec2 p0, glm::vec2 p1, int steps)
{
	...
	VkvgPattern g = dc_pattern_create_linear(ctx, rect.x, rect.y, rect.z, rect.w);
	--steps;
...
void paint_shadow_v(vgdev_ctx* ctx, double size_x, double size_y, double width, double height, const glm::vec4& shadow, bool rev, float r)
{
	auto trans = shadow;
	trans.w = 0;
	auto ph = ctx->get_path();
	auto gr = rev ?
		new_cubic_gradient(ctx, glm::vec4(0, 0, size_x, size_y), trans, shadow, glm::vec2(0.5, 0.0), glm::vec2(1.0, 0.5))
		: new_cubic_gradient(ctx, glm::vec4(0, 0, size_x, size_y), shadow, trans, glm::vec2(0, 0.5), glm::vec2(0.5, 1));

	dc_rectangle(ph, 0, 0, width, height, r);
	dc_set_source(ctx, gr);
	ctx->fill(ph);
}
void paint_shadow_v(vgdev_ctx* ctx, double size_x, double size_y, double width, double height, const glm::vec4& shadow, const glm::vec4& color_to, bool rev, float r)
{
	auto ph = ctx->get_path();
	auto gr = rev ?
		new_cubic_gradient(ctx, glm::vec4(0, 0, size_x, size_y), color_to, shadow, glm::vec2(0.5, 0.0), glm::vec2(1.0, 0.5))
		: new_cubic_gradient(ctx, glm::vec4(0, 0, size_x, size_y), shadow, color_to, glm::vec2(0, 0.5), glm::vec2(0.5, 1));
...
void rvg_x::fill()
{
	ctx->fill(ctx->get_path());
}
void rvg_x::stroke()
{
	ctx->stroke(ctx->get_path());
}
void rvg_x::fill_preserve() {
	ctx->fill_preserve(ctx->get_path());
}
void rvg_x::stroke_preserve() {
	ctx->stroke_preserve(ctx->get_path());
}
void rvg_x::fill_stroke(uint32_t fill, uint32_t color) {
	auto ph = ctx->get_path();
	if (fill)
	{
		ctx->fill_preserve(ph);
		if (!color)
			dc_clear_path(ph);
	}
	if (color)ctx->stroke(ph);
}
void rvg_x::set_line_width(float w) {
	dc_set_line_width(ctx, w);
}
void rvg_x::set_color(uint32_t color) {
	dc_set_color(ctx, color);
}
void rvg_x::set_color(const glm::vec4& rgba) {
	dc_set_color4(ctx, rgba);
}
...
void glist_cx::insert(size_t pos, geom_cmd_t* c)
{
	if (c) {
		cmd_t c1 = {}; c1.stype = 0; c1.c.g = *c;
		if (pos < cmdlist.size())
			cmdlist.insert(cmdlist.begin() + pos, c1);
		else
			cmdlist.push_back(c1);
	}
}
void glist_cx::insert(size_t pos, vgcmd_t* c)
{
	if (c) {
		cmd_t c1 = {}; c1.stype = 1; c1.c.v = *c;
		if (pos < cmdlist.size())
			cmdlist.insert(cmdlist.begin() + pos, c1);
		else
			cmdlist.push_back(c1);
...
void glist_cx::push(geom_cmd_t* c)
{
	cmd_t c1 = {}; c1.stype = 0; c1.c.g = *c;
	cmdlist.push_back(c1);
}
void glist_cx::push(vgcmd_t* c)
{
	cmd_t c1 = {}; c1.stype = 1; c1.c.v = *c;
	cmdlist.push_back(c1);
}
